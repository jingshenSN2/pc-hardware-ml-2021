...
cur = db.cursor()
today = int(time.time())
cur.execute(f"INSERT OR REPLACE INTO last_update (catagory, last_update_timestamp) VALUES ('cpu', {today})")

def get_cpu(manufacturer, model_name, ASIN_list):
    """
    :param manufacturer: 'intel' or 'amd',  manufacturer of cpu, as part of save path
    :param model_name: cpu product name
    ...
    for ASIN in ASIN_list:
        ...
        with open(f'../../data_gather/amazon/cpu/{manufacturer}/{model_name}-{ASIN}.json', 'rb') as f:
            json_file = json.load(f)
            product = json_file['products'][0]
            cur.execute(f"INSERT OR REPLACE INTO cpu (asin, catagory, manufacturer, brand, model_name, product_name, last_update) VALUES "
                        f"('{ASIN}', 'cpu', '{product['manufacturer']}', '{product['brand']}', '{model_name}', '{product['title']}', {int(product['lastUpdate'])})")
            ...
            for idx, csv in enumerate(csv_list):
                ...
                if 18 <= idx <= 29:
                    for i in range(0, len(csv), 3):
                        ...
                        cur.execute(
                            f"INSERT OR REPLACE INTO cpu_ts (asin, keepa_minute, keepa_idx, value1, value2) VALUES "
                            f"('{ASIN}', {timestamp}, {idx}, {value1}, {value2})")
                else:
                    for i in range(0, len(csv), 2):
                        timestamp, value = csv[i], csv[i + 1]
                        cur.execute(f"INSERT OR REPLACE INTO cpu_ts (asin, keepa_minute, keepa_idx, value1, value2) VALUES "
