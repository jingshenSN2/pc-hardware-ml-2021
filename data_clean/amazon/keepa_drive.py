...
def get_drive(manufacturer, drive_type, sub_type, model_name, ASIN_dict):
    ...
    :param model_name: ssd product name
    ...
    for capacity, ASIN in ASIN_dict.items():
        ...
        with open(f'../../data_gather/amazon/{drive_type}/{manufacturer}/{sub_type}-{model_name}-{capacity}-{ASIN}.json', 'rb') as f:
            ...
            title = product['title'].replace("'", ' ') if product['title'] else 'null'
            cur.execute(f"INSERT OR REPLACE INTO drive (asin, catagory, manufacturer, brand, model_name, product_name, drive_type, drive_subtype, capacity, last_update) VALUES "
                        f"('{ASIN}', 'drive', '{product['manufacturer']}', '{product['brand']}', '{model_name}', '{title}', '{drive_type}', '{sub_type}', '{capacity}', {int(product['lastUpdate'])})")
            csv_list = product['csv']
            for idx, csv in enumerate(csv_list):
                if not csv:
                    continue
                if 18 <= idx <= 29:
                    for i in range(0, len(csv), 3):
                        timestamp, value1, value2 = csv[i], csv[i + 1], csv[i + 2]
                        cur.execute(
                            f"INSERT OR REPLACE INTO drive_ts (asin, keepa_minute, keepa_idx, value1, value2) VALUES "
                            f"('{ASIN}', {timestamp}, {idx}, {value1}, {value2})")
                else:
                    for i in range(0, len(csv), 2):
                        timestamp, value = csv[i], csv[i + 1]
                        cur.execute(f"INSERT OR REPLACE INTO drive_ts (asin, keepa_minute, keepa_idx, value1, value2) VALUES "
                                    f"('{ASIN}', {timestamp}, {idx}, {value}, null)")
...
# get_hdd('WD', '35', 'wd red plus', {'2tb': 'B008JJLZ7G', '3tb': 'B008JJLW4M', '4tb': 'B08VH8C3WZ', '6tb': 'B00LO3KR96', '8tb': 'B07D3MWMNZ', '10tb': 'B083JXTH5L', '12tb': 'B07RQ99XJH', '14tb': 'B07YFGMZVV'})
...
# get_hdd('WD', '35', 'wd black', {'500gb': 'B008968L6M', '2tb': 'B00FJRS628', '4tb': 'B0792G331G', '6tb': 'B0792GSD6N', '8tb': 'B08GTXJ3LS', '10tb': 'B08MKJPFZ7'})
