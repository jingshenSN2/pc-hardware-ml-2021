...
def get_gpu(manufacturer, model_name, ASIN_list):
    ...
    for ASIN in ASIN_list:
        ...
        with open(f'../../data_gather/amazon/gpu/{manufacturer}/{model_name}-{ASIN}.json', 'rb') as f:
            json_file = json.load(f)
            product = json_file['products'][0]
            cur.execute(f"INSERT OR REPLACE INTO gpu (asin, catagory, manufacturer, brand, model_name, product_name, last_update) VALUES "
                        f"('{ASIN}', 'gpu', '{model_name.split(' ')[0]}', '{product['brand']}', '{model_name}', '{product['title']}', {int(product['lastUpdate'])})")
            ...
            for idx, csv in enumerate(csv_list):
                ...
                if 18 <= idx <= 29:
                    for i in range(0, len(csv), 3):
                        ...
                        cur.execute(
                            f"INSERT OR REPLACE INTO gpu_ts (asin, keepa_minute, keepa_idx, value1, value2) VALUES "
                            f"('{ASIN}', {timestamp}, {idx}, {value1}, {value2})")
                else:
                    for i in range(0, len(csv), 2):
                        timestamp, value = csv[i], csv[i + 1]
                        cur.execute(f"INSERT OR REPLACE INTO gpu_ts (asin, keepa_minute, keepa_idx, value1, value2) VALUES "
